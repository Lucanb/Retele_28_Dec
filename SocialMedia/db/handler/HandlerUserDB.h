...
*/
  ...
  /// Aici trebuie sa vad daca exista utilizatorul meu. Altfel returnez iar cv
        ...
        string sqlVerif="SELECT EXISTS(SELECT id FROM Users WHERE id=Users.id)"; ///cauta dupa id.(eu cautam dupa userName)
...
/// Test : nu prea e oke
 /*
         ...
         else
         {
            ...
            {
            */
                sqlVerif = "SELECT isAdmin,userName,firstname,lastname,birthday,accountCreationDate,profileDescription  FROM Users WHERE id = Users.id";  ///cauta dupa id in loc de userName.
             //era int rc
