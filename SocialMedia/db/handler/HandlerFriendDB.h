        ...
        if (rc != SQLITE_OK) {
            ...
            if (s.size() == 0) {
                ...
                string sqlQuery = "SELECT userName FROM Users JOIN Friend ON" + string("(") +
                                  "Users.id = Friend.id1 AND Friend.id2 = " + to_string(id) +
                                  ") "; //aici scot toti userii care au id ul 1 in Friends
    ...
    int updateFriend(Friend friendd) {
        ...
        if (rc != SQLITE_OK) {
            ...
            if (s.size() == 0) {
                ...
                string sqlQuery =
                        "UPDATE Friend SET type =" + string("\'") + friendd.type + string("\'") + "WHERE id1 = " +
                        to_string(friendd.id1) + " AND id2 = " + to_string(friendd.id2);
                string sqlQuery2 =
                        "UPDATE Friend SET type =" + string("\'") + friendd.type + string("\'") + "WHERE id1 = " +
                        to_string(friendd.id2) + " AND id2 = " + to_string(friendd.id1);
